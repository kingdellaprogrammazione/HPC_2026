...
\author[Allione, Frulla, Porro]{Paolo Allione \and Edoardo Frulla \and Michele Porro}
\institute[PoliTO]{Quantum Engineering -- High Performance Computing\\Academic Year 2025--26}
\date{\today}
...
\begin{frame}{Numerical discretization}
  ...
  \[
    \nabla^2 u \approx \frac{1}{6\Delta x^2}\Big[4(\text{edge neighbours}) + (\text{diagonal neighbours}) - 20\,u\Big]
  \]
  \textbf{Update rule (leapfrog scheme):}
  \[
    u^{n+1}_{i,j} = \frac{2u^n_{i,j} - \left(1-\dfrac{\gamma\Delta t}{2}\right)u^{n-1}_{i,j} + c^2\Delta t^2 \nabla^2 u^n_{i,j}}{1+\dfrac{\gamma\Delta t}{2}}
  \]
  \begin{itemize}
    \item Requires the two previous states $u^n$ and $u^{n-1}$ to compute $u^{n+1}$
...
\begin{frame}{Why a 9-point stencil?}
  \begin{columns}
    ...
    \begin{column}{0.45\textwidth}
      ...
      \[
        \underbrace{\frac{c\Delta t}{\Delta x} \le \frac{1}{\sqrt{2}} \approx 0.707}_{\text{5-point}}
      \]
      \[
        \underbrace{\frac{c\Delta t}{\Delta x} \le \frac{\sqrt{3}}{2} \approx 0.866}_{\text{9-point}}
      \]
  ...
  \begin{columns}
    ...
    \begin{column}{0.5\textwidth}
      \begin{itemize}
        ...
        \item Memory: $O(M^2)$, independent of $N$ (frames streamed to disk incrementally)
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Parallelization strategy}
  \begin{itemize}
    ...
    \item \texttt{\#pragma omp parallel for schedule(static)} over rows:
      \begin{itemize}
        \item identical per-row workload $\Rightarrow$ static preferred over dynamic
...
\begin{frame}{Results: time, speedup, efficiency}
  ...
  \begin{alertblock}{Observation}
    Measured speedup of $1.56\times$ against an ideal $48\times$: something is not scaling.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Hilbert curve analysis}
  \begin{columns}
    \begin{column}{0.55\textwidth}
      ...
      \end{itemize}
      \textbf{Result:} absolute cache misses are slightly \emph{higher}
      with Hilbert; the \emph{relative} miss rate is essentially
      identical to row-major.
    \end{column}
    \begin{column}{0.45\textwidth}
      \begin{itemize}
        \item Tested on a local laptop, L1 = 256 KiB $\Rightarrow$ 32768
              doubles fit in cache: multiple rows already coexist
...
\begin{frame}{The color transfer function}
  ...
  \]
  \textbf{Step 2 -- diverging color map:} calm $\to$ white, crest ($d{=}{+}1$) $\to$ red, trough ($d{=}{-}1$) $\to$ blue
...
\begin{frame}{Final RGB and CUDA kernel}
  ...
  \]
  \begin{itemize}
    \item Kernel launched with \textbf{one thread per pixel}, $16\times16$ blocks
