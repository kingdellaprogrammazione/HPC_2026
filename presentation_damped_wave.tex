\documentclass[11pt,aspectratio=169]{beamer}

% ---------- Theme ----------
\usetheme{Madrid}
\usecolortheme{default}
\setbeamertemplate{navigation symbols}{}
\setbeamertemplate{footline}[frame number]
\setbeamertemplate{caption}[numbered]

\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{amsmath,amssymb}
\usepackage{booktabs}
\usepackage{graphicx}
\usepackage{tikz}
\usetikzlibrary{arrows.meta,positioning,calc}
\usepackage{listings}
\usepackage{xcolor}
\usepackage{pgf}

\def\mathdefault#1{#1}

\lstset{
  basicstyle=\ttfamily\scriptsize,
  keywordstyle=\color{blue},
  commentstyle=\color{gray},
  breaklines=true,
  frame=single,
  backgroundcolor=\color{gray!5}
}

\title[Parallel Simulation of the Damped Wave Equation]
{Parallel Simulation of the Damped Wave Equation}
\subtitle{A Comparative Study Using OpenMP, MPI, and CUDA}
\author[Allione, Frulla, Porro]{Paolo Allione \and Edoardo Frulla
\and Michele Porro}
\institute[PoliTO]{Quantum Engineering -- High Performance
Computing\\Academic Year 2025--26}
\date{\today}

\begin{document}

% ================= Slide 1 =================
\begin{frame}
  \titlepage
\end{frame}

% ================= Slide 2: Agenda =================
\begin{frame}{Outline}
  \tableofcontents
\end{frame}

% ================= SECTION: Introduction =================
\section{Introduction}

\begin{frame}{The physical problem}
  \textbf{Damped wave equation (2D):}
  \[
    \frac{\partial^2 u}{\partial t^2} + \gamma \frac{\partial
    u}{\partial t} = c^2 \nabla^2 u
  \]
  \begin{itemize}
    \item $u(x,y,t)$: wave height on the plane $(x,y)$ at time $t$
    \item $\gamma$: damping coefficient (energy dissipation)
    \item $c$: wave propagation speed
  \end{itemize}
  \vspace{0.3cm}
  Compared to the ideal, non-dissipative wave equation, the term
  $\gamma \, \partial u/\partial t$ progressively attenuates the
  amplitude as the wave propagates.
\end{frame}

\begin{frame}{Numerical discretization}
  \textbf{Laplacian (9-point isotropic stencil):}
  \[
    \nabla^2 u \approx \frac{1}{6\Delta x^2}\Big[4(\text{edge
    neighbours}) + (\text{diagonal neighbours}) - 20\,u\Big]
  \]
  \textbf{Update rule (leapfrog scheme):}
  \[
    u^{n+1}_{i,j} = \frac{2u^n_{i,j} - \left(1-\dfrac{\gamma\Delta
      t}{2}\right)u^{n-1}_{i,j} + c^2\Delta t^2 \nabla^2
    u^n_{i,j}}{1+\dfrac{\gamma\Delta t}{2}}
  \]
  \begin{itemize}
    \item Requires the two previous states $u^n$ and $u^{n-1}$ to
      compute $u^{n+1}$
    \item Computational core shared by all three parts of the project
  \end{itemize}
\end{frame}

% ================= SECTION: Part 1 =================
\section{Part 1 -- OpenMP}

\begin{frame}{Why a 9-point stencil?}
  \begin{columns}
    \begin{column}{0.55\textwidth}
      \begin{itemize}
        \item The classic \textbf{5-point} stencil has a non-isotropic
          truncation error $\Rightarrow$ diamond/square-shaped wavefront
        \item The \textbf{9-point} stencil blends axial and diagonal
          neighbours $\Rightarrow$ much more circular wavefronts
        \item Same $O(\Delta x^2)$ accuracy
      \end{itemize}
    \end{column}
    \begin{column}{0.45\textwidth}
      \textbf{Stability condition (CFL):}
      \[
        \underbrace{\frac{c\Delta t}{\Delta x} \le \frac{1}{\sqrt{2}}
        \approx 0.707}_{\text{5-point}}
      \]
      \[
        \underbrace{\frac{c\Delta t}{\Delta x} \le \frac{\sqrt{3}}{2}
        \approx 0.866}_{\text{9-point}}
      \]
      Surprisingly \textbf{less restrictive}!
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}{Parameters and complexity}
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{table}
        \centering\small
        \begin{tabular}{lc}
          \toprule
          Parameter & Value \\
          \midrule
          $\gamma$ (damping) & 0.152 \\
          $c$ (speed) & 0.55 \\
          $\Delta t$ & 0.0083 \\
          $\Delta x$ & 0.025 \\
          $M$ (grid side) & 2000 \\
          $N$ (\# frames) & 3840 \\
          \bottomrule
        \end{tabular}
      \end{table}
    \end{column}
    \begin{column}{0.5\textwidth}
      \begin{itemize}
        \item Cost per frame: $O(M^2)$
        \item Total cost: $O(N M^2)$
        \item With $P$ OpenMP threads: $O(NM^2/P)$ (ideal case)
        \item 3840 frames at 120 fps $\Rightarrow$ a 32 s video
        \item Memory: $O(M^2)$, independent of $N$ (frames streamed
          to disk incrementally)
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}{Parallelization strategy}
  \begin{itemize}
    \item Each interior point writes only to \texttt{new}, reads from
      \texttt{old}/\texttt{current} $\Rightarrow$ no synchronization
      within the loop body
    \item \texttt{\#pragma omp parallel for schedule(static)} over rows:
      \begin{itemize}
        \item identical per-row workload $\Rightarrow$ static
          preferred over dynamic
        \item good cache locality, no false sharing
      \end{itemize}
    \item Thread counts tested: $T \in \{1,2,4,8,16,32,48\}$
    \item \texttt{OMP\_PROC\_BIND=true}, \texttt{OMP\_PLACES=cores} for pinning
  \end{itemize}
\end{frame}

\begin{frame}{Results: time, speedup, efficiency}
  \begin{table}
    \centering\small
    \begin{tabular}{cccc}
      \toprule
      Threads & Time (s) & Speedup & Efficiency (\%) \\
      \midrule
      1  & 360.37 & 1.00 & 100.0 \\
      2  & 348.30 & 1.04 & 51.7 \\
      4  & 292.52 & 1.23 & 30.8 \\
      8  & 298.50 & 1.21 & 15.1 \\
      16 & 274.05 & 1.32 & 8.2 \\
      32 & 239.67 & 1.50 & 4.7 \\
      48 & 231.22 & \textbf{1.56} & 3.2 \\
      \bottomrule
    \end{tabular}
  \end{table}
  \begin{alertblock}{Observation}
    Measured speedup of $1.56\times$ against an ideal $48\times$:
    something is not scaling.
  \end{alertblock}
\end{frame}

\begin{frame}{The bottleneck: I/O}
  \begin{itemize}
    \item Effective CPU Utilization: $<1\%$ for $T\le 8$, only $5.2\%$ at $T=48$
    \item \textbf{CPU time} grows with $T$ (aggregate work), but
      \textbf{wait time} stays roughly constant ($\sim$47--59 s)
    \item Serial PGM frame writing (\texttt{write\_snapshot\_serial})
      is a fixed cost that thread count cannot reduce
  \end{itemize}
  \begin{block}{Amdahl's Law}
    The simulator is \textbf{I/O-bound}, not compute-bound: the serial
    write fraction places a hard ceiling on achievable speedup,
    confirmed empirically ($1.56\times$ observed).
  \end{block}
\end{frame}

\begin{frame}{Hilbert curve analysis}

  \begin{itemize}
    \item Idea: reorder the 2D grid traversal along a Hilbert curve
      to improve spatial locality for the 9-point stencil
      (which spans 3 rows)
    \item Expectation: points close in 2D stay close in the 1D array
  \end{itemize}
  \begin{figure}
    \centering
    \scalebox{0.8}{\input{./damped_wave/openmp/images/hilbert_drawing.pgf}}
  \end{figure}

\end{frame}

\begin{frame}[fragile]{9-Point Hilbert Wave Update}
  \scriptsize
\begin{lstlisting}[language=C]
double wave_update_9_pts_hilbert(...)
{
    /* 9-point isotropic Laplacian */
    double lap = (
        /* corners (weight 1) */
        curr[H[(i-1)*M + (j-1)]] + curr[H[(i-1)*M + (j+1)]] +
        curr[H[(i+1)*M + (j-1)]] + curr[H[(i+1)*M + (j+1)]] +

        /* edge neighbors (weight 4) */
        4.0 * curr[H[(i-1)*M + j]] +
        4.0 * curr[H[(i+1)*M + j]] +
        4.0 * curr[H[i*M + (j-1)]] +
        4.0 * curr[H[i*M + (j+1)]] +

        /* center (weight -20) */
        -20.0 * curr[H[i*M + j]]
    ) * (inv_dx2 / 6.0);

    /* ... */
}
\end{lstlisting}
\end{frame}

\begin{frame}{(Unexpected) Result}

  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{figure}
        \centering
        \scalebox{0.7}{\input{./damped_wave/openmp/images/hilbert_cache_misses_comparison.pgf}}
      \end{figure}

    \end{column}

    \begin{column}{0.5\textwidth}
      \begin{figure}
        \centering
        \scalebox{0.7}{\input{./damped_wave/openmp/images/hilbert_L1_miss_rate.pgf}}
      \end{figure}

    \end{column}
  \end{columns}
  \vspace{10pt}
  \begin{itemize}
    \item Tested on a local laptop, L1 = 256 KiB $\Rightarrow$ 32768
      doubles fit in cache: multiple rows already coexist ($M = 1024$)
    \item The effect might emerge on larger grids
  \end{itemize}

\end{frame}

% ================= SECTION: Part 2 =================
\section{Part 2 -- MPI}

\begin{frame}{Multi-simulation extension}
  Three concurrent simulations (hybrid MPI+OpenMP):
  \begin{enumerate}
    \item \textbf{Sim 1}: single source at $(i_1,j_1)$, $t=0$
    \item \textbf{Sim 2}: Sim 1 + a second source at $(i_2,j_2)$, both
      activated at $t=0$ $\to$ symmetric interference
    \item \textbf{Sim 3}: like Sim 2, but the second source starts at
      $t_{\text{start}}=8.3 > 0$ $\to$ asymmetric interference
  \end{enumerate}
  \vspace{0.2cm}
  Same $\gamma=0.152$, $c=0.55$ for all waves; intensities
  $-37, +92, -47$ respectively.
\end{frame}

\begin{frame}{Two MPI strategies compared}
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \textbf{Independent simulations}
      \begin{itemize}
        \item Each rank computes its own simulation in full
        \item Sim 1 is recomputed 3 times (wasted work)
        \item Inter-node communication $\approx 0$
        \item Optimized contrast: color range based on
          $[-|I_1|-|I_i|,\,|I_1|+|I_i|]$
      \end{itemize}
    \end{column}
    \begin{column}{0.5\textwidth}
      \textbf{Dependent simulations}
      \begin{itemize}
        \item Rank 0 computes Sim 1 and sends it
          (\texttt{MPI\_Send/Recv}) to the other ranks every frame
        \item Exploits linearity of the equation: sum of solutions
          = solution
        \item Avoids recomputing Sim 1, but introduces per-frame
          communication
      \end{itemize}
    \end{column}
  \end{columns}
  \vspace{1cm}
  \centering
  \small \texttt{SLURM}: 3 nodes, 1 task/node, 48 cores/task.
\end{frame}

\begin{frame}[fragile]{Independent Simulations}
  \scriptsize
\begin{lstlisting}[language=C]
    // Read remaining parameters
    MPI_Init()
    /* ... */

    if (rank == 0) {
          // Simulate first wave
              /* ... */
    }
    if (rank == 1) {
          // Simulate first and second wave
              /* ... */
    }
    if (rank == 0) {
          // Simulate first and third wave
              /* ... */
    }

    /* ... */
    MPI_Finalize()
\end{lstlisting}
\end{frame}

\begin{frame}{Independent Simulations}
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{figure}
        \centering
        \scalebox{0.7}{\input{./damped_wave/MPI/images/mpi_scaling_48_independent.pgf}}
      \end{figure}

    \end{column}

    \begin{column}{0.5\textwidth}
      \begin{figure}
        \centering
        \scalebox{0.7}{\input{./damped_wave/MPI/images/no_writing/mpi_scaling_48_independent.pgf}}
      \end{figure}

    \end{column}
  \end{columns}

\end{frame}

\begin{frame}[fragile]{Dependent Simulations}
  \scriptsize
\begin{lstlisting}[language=C]
TO BE FINISHED

    /* ... */
\end{lstlisting}
\end{frame}

\begin{frame}{Dependent Simulations}
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{figure}
        \centering
        \scalebox{0.7}{\input{./damped_wave/MPI/images/mpi_scaling_48_coordinated.pgf}}
      \end{figure}

    \end{column}

    \begin{column}{0.5\textwidth}
      \begin{figure}
        \centering
        \scalebox{0.7}{\input{./damped_wave/MPI/images/no_writing/mpi_scaling_48.pgf}}
      \end{figure}

    \end{column}
  \end{columns}

\end{frame}

\begin{frame}{Results: scaling with grid size}
  \begin{itemize}
    \item Execution time $\sim O(M^2)$ for both strategies
      (consistent with the per-frame cost)
    \item Concurrent writes to a serial filesystem: \textbf{5$\times$
      higher} running time compared to computation alone
    \item VTune (independent strategy, $M=1024$):
      \texttt{wave\_update\_9\_pts} dominates (56.3\% CPU time),
      but effective core utilization $\approx 5.2\%$
  \end{itemize}
  \begin{exampleblock}{Comparison with Part 1}
    At $M=2048$, the time for 3 MPI simulations $\approx$ the time of
    \textbf{a single} sequential Part 1 simulation
    $\Rightarrow$ speedup $\approx 3\times$.
  \end{exampleblock}
\end{frame}

\begin{frame}{Independent vs. dependent}
  \begin{itemize}
    \item For small $M$: the \emph{dependent} strategy (communication)
      gives a small advantage
    \item For large $M$: the \emph{independent} strategy wins
      (computation is faster than communication + summation)
    \item Dependent strategy: CPU time dominated by
      \texttt{gomp\_simple\_barrier\_wait} (59.8\%) $\to$ heavy waiting
    \item No difference in complexity order between the two
      strategies: both $O(M^2)$
  \end{itemize}
\end{frame}

% ================= SECTION: Part 3 =================
% ================= SECTION: Part 3 =================
\section{Part 3 -- CUDA}

  \begin{frame}{Part 3 -- GPU colorization}
  The simulation is unchanged: it still produces one \textbf{grayscale}
  frame per time step. The goal of Part~3 is purely visual.
  \begin{itemize}
    \item Turn each grayscale frame into a \textbf{color} image
    \item Make crests and troughs stand out, keep calm areas white
    \item Do the recoloring on the \textbf{GPU} using CUDA
    \item Save each frame as \texttt{frame\_<xxxxx>.ppm} 
  \end{itemize}
  \vspace{0.2cm}
  Key point: the wave keeps evolving on the original grayscale matrix;
  the GPU only produces a \emph{colored copy} for the output.
\end{frame}

\begin{frame}{From grayscale to color: the transfer function}
  Coloring is \textbf{embarrassingly parallel}, each pixel's color
  depends only on itself, so it runs on the GPU with one thread per
  pixel. Each thread maps a grayscale value $g$ to an $(R,G,B)$ triplet
  in three steps.
  \vspace{0.3cm}

  \textbf{Step 1 -- signed deviation.} Re-center the gray value so that
  its sign tells crest from trough:
  \[
    d = \frac{g-127.5}{127.5} \in [-1,+1]
  \]

  \textbf{Step 2 -- diverging color map.} The sign of $d$ picks the color:
  calm $\to$ white, crest ($d{=}{+}1$) $\to$ red, trough ($d{=}{-}1$)
  $\to$ blue.

  \textbf{Step 3 -- nonlinear correction.} The wave is damped and decays
  by 3 orders of magnitude, so a linear map would fade to white. A power
  law fixes this:
  \[
    d' = \text{sign}(d)\,|d|^{\gamma_c}, \qquad \gamma_c = 0.30
  \]
  It amplifies the weak fronts without reordering the values.
\end{frame}

\begin{frame}{Final RGB and CUDA kernel}
  The corrected value $d'$ is turned into a color by interpolating between
  white and the saturated extreme picked by its sign:
  \[
    (R,G,B) =
    \begin{cases}
      (255,\ 255(1-d'),\ 255(1-d')) & d' \ge 0 \quad\text{(white}\to\text{red)}\\
      (255(1+d'),\ 255(1+d'),\ 255) & d' < 0 \quad\text{(white}\to\text{blue)}
    \end{cases}
  \]
  For a crest one channel stays at 255 while the others fade from 255 to 0,
  giving a smooth white-to-red (or white-to-blue) gradient with no banding.
  \vspace{0.3cm}

  This function is evaluated by the \textbf{CUDA kernel}:
  \begin{itemize}
    \item Launched with \textbf{one thread per pixel}, in $16\times16$
      blocks; each thread finds its own pixel from its block/thread index
    \item Each thread reads its grayscale value, applies the equations,
      and writes the $(R,G,B)$ triplet to the interleaved output buffer
    \item Pixels are independent $\to$ \textbf{no synchronization} needed
 
    
  \end{itemize}
\end{frame}

\begin{frame}{Colorized output}
  \begin{figure}
    \centering
    \includegraphics[width=0.31\linewidth]{ph1.png}\hfill
    \includegraphics[width=0.31\linewidth]{ph2.png}\hfill
    \includegraphics[width=0.31\linewidth]{ph3.png}
    \caption{Colorized frames over time: an initial blue trough, then a
    red crest with a blue ring, then the wavefront expanding and fading
    as the wave is damped.}
  \end{figure}
  \vspace{0.1cm}
  Troughs are blue, crests are red, calm regions white. 
\end{frame}

\begin{frame}{The bottleneck: moving data, not computing it}
  Each frame is a three-stage pipeline: copy the grayscale image to the
  GPU (H2D), color it, copy the result back (D2H). Measuring the three
  stages separately reveals where the time goes:
  \begin{itemize}
    \item The \textbf{kernel is extremely cheap}: $\sim 0.01$ ms at
      $M=512$, only $\sim 0.06$ ms at $M=2048$
    \item The two \textbf{transfers dominate}: 85--90\% of device-side time
  \end{itemize}
  \vspace{0.2cm}
  \begin{alertblock}{The real cost is data movement}
    For this problem the arithmetic is nearly free; the bottleneck is
    moving data across the PCIe bus between CPU and GPU. 
  \end{alertblock}
\end{frame}

\begin{frame}{Optimization: pinned host memory}
  \textbf{The problem.} Memory from \texttt{malloc} is \emph{pageable}:
  the OS can move it around in RAM at any time. But the GPU's DMA engine
  needs the data at a \textbf{fixed} physical address. Since pageable
  memory can move, the GPU cannot read it directly, the driver first
  copies it into a temporary locked buffer, then transfers it.
  \textbf{Every byte is copied twice.}
  \vspace{0.3cm}

  \textbf{The fix.} Memory from \texttt{cudaHostAlloc} is \emph{pinned}
  (page-locked): nailed to a fixed address, it never moves. The DMA
  engine reads it directly, the intermediate copy disappears, and
  \textbf{every byte is copied once.}
  \vspace{0.3cm}

  \begin{block}{Controlled experiment}
    Both builds come from the \textbf{same source}, differing only by a
    compile flag. The kernel time stays identical, confirming that pinned
    memory affects only the transfers, not the computation.
  \end{block}
\end{frame}

\begin{frame}{Results: pinned vs pageable}
  Pinned memory nearly doubles the effective bandwidth, from $\sim$14 GB/s
  to $\sim$25 GB/s, close to the practical PCIe limit.
  \vspace{0.2cm}
  \begin{table}
    \centering\small
    \begin{tabular}{ccccc}
      \toprule
      $M$ & pageable [ms] & pinned [ms] & speedup & BW pinned [GB/s] \\
      \midrule
      128  & 0.0412 & 0.0336 & 1.23$\times$ & 3.72 \\
      512  & 0.1878 & 0.0873 & \textbf{2.15$\times$} & 21.22 \\
      2048 & 1.9168 & 1.1678 & 1.64$\times$ & 24.68 \\
      \bottomrule
    \end{tabular}
  \end{table}
  \vspace{0.1cm}
  \begin{block}{Why the speedup is not constant}
    Small $M$: fixed per-call overhead dominates (pinning doesn't help).
    Medium $M$: bandwidth dominates $\to$ pinning helps most (peak at
    $M{=}512$). Large $M$: pinned bandwidth saturates near the PCIe limit.
  \end{block}
\end{frame}




% ================= SECTION: Conclusions =================
\section{Conclusions}

\begin{frame}{Summary of the three paradigms}
  \begin{table}
    \centering\small
    \begin{tabular}{lll}
      \toprule
      Paradigm & Bottleneck & Key lesson \\
      \midrule
      OpenMP & Serial I/O (frame writing) & Amdahl: computation scales,
      \\ & & writing does not \\
      MPI+OpenMP & Shared filesystem / & Trade-off between recomputation\\
      & per-frame communication & and communication \\
      CUDA & PCIe H2D/D2H transfers & Kernel is nearly free; data
      \\ & & movement is the real cost \\
      \bottomrule
    \end{tabular}
  \end{table}
  \vspace{0.3cm}
  In all three cases, the raw computation (stencil, kernel) scales
  very well: the real limit lies in \textbf{data movement} (disk,
  network, PCIe bus).
\end{frame}

\begin{frame}{Conclusions}
  \begin{itemize}
    \item \textbf{OpenMP}: correct and efficient shared-memory
      baseline, but limited speedup ($1.56\times$ on 48 threads)
      due to serial disk writing
    \item \textbf{MPI+OpenMP}: concurrent execution of independent
      simulations, speedup $\approx 3\times$ over sequential
      execution; multi-source interference correctly modeled
    \item \textbf{CUDA}: effective GPU-accelerated colorization; extra
      optimization with pinned memory up to $2.15\times$ on transfers
    \item \textbf{Future work}: asynchronous/buffered I/O, reduced
      snapshot frequency, compute-transfer overlap
  \end{itemize}
\end{frame}

\begin{frame}
  \centering
  \Huge Thank you for your attention
  \vspace{0.5cm}

  \large Questions?
\end{frame}

\end{document}
