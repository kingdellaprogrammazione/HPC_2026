...
versus $8/\Delta x^2$ for the 5-point stencil; substituting into the
standard leapfrog stability bound
...
\begin{table}[h]
  \centering
  \footnotesize
  \caption{Execution time, speedup, efficiency, and CPU utilization
  vs.\ OpenMP thread count ($M=2000$, $N=3840$).}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.32\linewidth}
    ...
    \caption{$t \approx N$}
  \end{subfigure}
  \caption{Example frames of the wave simulation at different time
  steps, showing propagation and progressive damping of the wave amplitude.}
...
\begin{figure}[h]
  ...
  \caption{Execution time, speedup, and parallel efficiency as a
  function of the number of OpenMP threads ($M=2000$, $N=3840$).}
...
\begin{figure}[h]
  ...
  \caption{CPU time (useful computation) vs.\ Wait time (I/O and
    synchronization) across thread counts, from VTune Hotspots and
  Threading collections.}
  \label{fig:part1-io}
\end{figure}

\input{part_1_caches.tex}